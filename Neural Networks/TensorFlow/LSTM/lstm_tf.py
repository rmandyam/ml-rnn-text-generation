...
class Config(object):
    ...
    embed_size  = 512 #1024 #512 #256 #128 #100 #80 #128 #80
    hidden_size = 512 #1024 #512 #256 #128 #100 #80 #128 #80
...
class LSTMModel(object):
   ...
   def load_data(self,debug=False):
	...
	self.train, self.word_embedding, self.max_sen_len, self.input_vocab_size, self.vocab, self.ivocab  =  utils.load_input(self.config)
        ...
        print '---> Config Number of LSTM Layers is..', self.config.number_of_layers
   ...
   def add_placeholders(self):
	...
	max_sen_len =  self.max_sen_len
        ...
        #For just ONE layer of LSTM
    	#self.lstm_cell = tf.contrib.rnn.BasicLSTMCell(size)     #state_is_tuple is True by default
    	#self._initial_state = self.lstm_cell.zero_state(self.config.batch_size, tf.float32)

        #For MORE THAN ONE layer of LSTM
    	self.lstm_cell = tf.contrib.rnn.BasicLSTMCell(size, state_is_tuple=False)
        self.stacked_lstm = tf.contrib.rnn.MultiRNNCell([self.lstm_cell] * self.config.number_of_layers, state_is_tuple=False)
    	self._initial_state = self.stacked_lstm.zero_state(self.config.batch_size, tf.float32)
   ...
   def inference(self):
        """Performs inference on the LSTM  model"""
	...
	#Uncomment for debugging.. 
        ...
        #For ONE LSTM layer
	#outputs,state=tf.nn.dynamic_rnn(self.lstm_cell, inputs,sequence_length=self.input_len_placeholder,initial_state= state)
	#For MORE THAN ONE LSTM layer
	outputs,state=tf.nn.dynamic_rnn(self.stacked_lstm, inputs,sequence_length=self.input_len_placeholder,initial_state= state)
