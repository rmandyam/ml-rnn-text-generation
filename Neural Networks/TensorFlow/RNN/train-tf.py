...
Usage : train-tf.py to train a model
      : train-tf.py --r RESTORE  to restore a trained model from an epoch and generate text
"""
...
from rnn_tf import Config
from rnn_tf import RNNModel
import argparse
...
with tf.Session() as session:
	...
	# BEFORE init, set batch size to 1 if restoring..
	    ...
	    #NOTE : make sure the data for the epoch exists in the weights directory
            ...
            #saver.restore(session, 'weights/epoch' + '9' + '.weights')

            num_sentences =  10 #20 #10 
            print '\n************ GENERATING TEXT **************************'
