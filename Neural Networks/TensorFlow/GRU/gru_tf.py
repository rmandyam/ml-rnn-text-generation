...
class GRUModel(object):
   ...
   def add_placeholders(self):
	...
	max_sen_len =  self.max_sen_len
        ...
        #For just ONE layer of GRU
    	self.gru_cell = tf.contrib.rnn.GRUCell(size)     #state_is_tuple is True by default
    	self._initial_state = self.gru_cell.zero_state(self.config.batch_size, tf.float32)
   ...
   def inference(self):
	...
	#Uncomment for debugging..
        ...
        #For ONE GRU layer
	outputs,state=tf.nn.dynamic_rnn(self.gru_cell, inputs,sequence_length=self.input_len_placeholder,initial_state= state)
